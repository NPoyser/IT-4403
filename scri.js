...
async function initAuth0() {
  auth0Client = await auth0.createAuth0Client({
    ...
    clientId: "FnGFATY4Ljk54MuGKgeyndbcOEBlR428",
    authorizationParams: {
...
$(document).ready(function () {
  ...
  function displayMovieDetails(movie) {
    ...
    const favIcon = isFavorite(movie.id) ? "&#9829; Favorited" : "&#9825; Add to Favourites";
  ...
  $(document).on("click", ".fav-btn-detail", function (e) {
    ...
    btn.html(nowFav ? "&#9829; Favorited" : "&#9825; Add to Favourites");
  ...
  $(document).on('click', '.view-more-btn', function () {
    const btn = $(this);
    const rowDiv = btn.prev('.person-row');
    rowDiv.find('.hidden-person').slideToggle();
  ...
  function searchMovies(query) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        let html = `<h2 class="category-title">Search results for "${query}"</h2><div class="movie-grid">`;
